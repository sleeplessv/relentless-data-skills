...
| [`dlt-bootstrap`](skills/dlt-bootstrap/) | Bootstrap a dlt ingestion project: install the dltHub AI Workbench project-scoped, then layer Relentless Data house conventions (Snowflake, Prefect, DuckDB dev loop) as a committed always-on rule. |
...
| [`smart-git-commit`](skills/smart-git-commit/) | Groups changed files by affected area, creates one conventional commit per group, then pushes to remote — conventional commit format with lowercase type prefixes. |
...
| [`visual-report`](skills/visual-report/) | Produce a single self-contained HTML visual report — an explainer, writeup, or diagram-heavy document built with Tailwind and Mermaid via CDN plus hand-crafted CSS/SVG. Use when the user wants a visual report, an explainer, a single-file HTML writeup of a system/process/findings, or asks to visualize or diagram something as a shareable document. |
<!-- skills-table:end -->
