...
- **Static preflight, once per session.** `scripts/preflight.py` (stdlib
  ...
  the failures that are cheapest to detect statically and most expensive
  to diagnose from a `JWT token is invalid` error. A `--connect` flag
  escalates to a live `dbt debug` when diagnosing.
...
- **Signature-indexed failure catalogue.** `references/failures.md` is
  ...
  e.g. a DNS error means *suspect the sandbox first*, not the credentials.
