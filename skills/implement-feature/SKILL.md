...
- **Every ticket dispatch gets worktree isolation, even a one-ticket wave.** This is deliberately stricter than orchestrator-mode's two-writer rule, so the main tree never leaves the integration branch. Use the delegation tool's native worktree isolation (orchestrator-mode reference, Tool Mapping); none available → stop and say so rather than sharing the main tree.
...
1. **Verify** is the step-0 feedback-loop commands: full type-check + test suite + runtime smoke check on the integration branch.
