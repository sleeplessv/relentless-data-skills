...
Everything else: claiming (step 1), environment setup before the baseline (the orchestrator dispatches you inside a fresh worktree, which starts without installed dependencies; run the project's install command after any branch work and before the step 4 baseline, and report the worktree's path as `worktree_path`), the types+tests loop, the smoke check, and the WIP push + findings comment on failure. This is unchanged. One stop-path exception: leave the assignment and labels untouched (no `needs-info` swap): the orchestrator owns the ticket's lifecycle and retries.
