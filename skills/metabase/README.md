...
The **`metabase`** agent skill: operate a Metabase instance through its **REST
API** via the `mb` wrapper — read the SQL behind a question, trace where a
dashboard number actually comes from, run read-only ad-hoc SQL against a
connected database, audit dashboard filter wiring, and edit cards and dashboards
under a restore-point gate.
...
to echo a secret — including not testing for one with `${VAR:-no}`, which prints
the value when set.
...
- `SKILL.md` — core: first-run routing, wrapper invocation, the source chain,
  the two field-reference orders, the mutation gate, read-only SQL discipline.
...
- `references/mutations.md` — the mutation playbook: editing, dashboard writes,
  repointing at another database, and the verification recipe.
- `references/docs-map.md` — upstream doc URLs, validated weekly by CI.
- `scripts/mb.py` — the wrapper (stdlib only): config resolution, transport, the
  read-only SQL check, restore points, and the wiring audit.
