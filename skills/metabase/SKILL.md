...
card JSON alone does not answer it — compile. Native SQL printed verbatim is
**not runnable as-is** either: `{{tag}}` and `[[…]]` are Metabase syntax, not
SQL — `mb sql-of <id> --compiled` substitutes them.
...
1. **Establish blast radius.** `mb deps <id> --deep` — which dashboards show it,
   which cards are built on it, which may be frozen copies. Report it first.
2. **Send only the fields you are changing.** `mb update-card <id> --file f.json`
   ...
   a dashboard `PUT` replaces the *entire* `dashcards` list — a short list
   deletes the rest, so it refuses one without `--allow-removal`.
...
Repointing at another database (six id carriers to remap), the dashboard
verification failures this skill has actually shipped, and the drift caused by
live source tables: [references/mutations.md](references/mutations.md) — read it
before any write.
...
production with extra care. **Project to the columns you need.** Reporting databases carry payload columns —
key material, blob/`data` columns, signed URLs — that a `SELECT *` or a bare
`mb run <id>` will dump into the transcript. Name your columns, and use
`mb run <id> --count` when the row count is the answer.
