...
Built-in specialists are minimal (Claude Code has `Explore`, `Plan`, `general-purpose`;
Cursor has `explore`, `bash`, `browser`; Cortex Code has `Explore`, `Plan`, `general-purpose`,
...
- The delegation tool is withheld at the depth limit **and from restricted subagent
  ...
  does), so designate coordinators only as an unrestricted general agent.
...
3. **Integrate.** After all writers return, one dedicated dispatch merges the branches.
   ...
   **escalates semantic ones**, reporting the conflicting branches/files/hunks so the
   ...
   coding tier, since semantic conflicts cross two writers' intents). On success it removes
