...
The `review-pbi-diff` agent skill turns a Power BI (PBIP) git diff, hundreds
of hash-named `visual.json` files and TMDL model files nobody can read, into
a single manager-ready review artifact. That artifact holds an executive
summary, to-scale page wireframes showing what changed where, every
added/modified measure spec-checked against the repo's metric definitions,
and a ranked red-flags list. It is built for a Power BI expert reviewing a
developer's branch.
...
- **Deterministic extraction first.** A bundled stdlib-only script
  ...
  `change_model.json`. It has per-page visual inventories with absolute
  geometry (visual-group offsets resolved), field bindings and filters,
  per-table measure/column/calculation-item diffs with full before/after DAX,
  ...
  diff. It supports `A...B`, `A..B`, single-commit, and base-vs-working-tree
...
- **Scratch-page exclusion.** Power BI default-named pages (`Page N`), plus
  ...
  work. They are excluded from wireframes, tables, and checks, and summarized
  in one muted exclusion line, with one guard: a scratch page left visible in
  view mode is flagged, because it ships to end users.
...
- **Hosted HTML artifact.** One self-contained file with wireframes as
